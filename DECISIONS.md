...
- **Failover (3):** when the first Groq key fails the call reaches the second Groq key; only if *both* fail does it raise; no keys configured raises clearly — guards against a key/account outage taking the whole system down.
...
3. **Fail fast between keys.** The LLM client waited up to 45s twice per key before failing over — so a degraded primary could eat the whole budget before the second Groq key was ever tried. Cut to a short per-call timeout with a straight fail-over to the backup key (the two independent keys are the redundancy; a slow inner retry on a stalling endpoint just burns the deadline).
...
**Backup — a second Groq key on an independent account.** The backup is a second Groq API key on a separate account: if the primary key is down or rate-limited, the exact same call re-issues against the second key. I verified the failover end-to-end by deliberately breaking the primary key and confirming the answer came back from the backup key. The point of the failover is key/account independence, so a single account's outage or rate-limit can't take the answer layer down.
