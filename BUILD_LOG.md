...
- **My decision: two Groq keys with failover.** Both LLMs try Groq key 1 first, then fall over to Groq key 2 on a separate account if the first is down/rate-limited — so a single key's outage can't take the system down. Only if both fail does the user see an honest "service unavailable" message. Logged in DECISIONS.md in my words. Failover unit-tested (key1-fails→key2-serves, all-fail→error, no-keys→error).
...
**Firm-type answers — my decision on the answer shape:**
- "List all multi-family offices" returned only 2 (single-family only 3). Not a bug — only 2 firms are proven MFO / 3 proven SFO; the other 45 are Unconfirmed (split never proven). But "2" reads as a thin dataset when we actually hold 47 relevant firms.
- **My call:** answer type questions in two parts in one reply — (1) the firms 100% confirmed as the asked type, then (2) below a separator, the verified firms whose single-vs-multi label isn't confirmed yet, stated plainly as correct/verified records just missing the tag. Honest and complete; never hides the 45, never fakes a type. Reasoning in DECISIONS.md (2026-07-28, type-questions entry).
- Built via a widened retrieval filter (type + Unconfirmed) + a two-section instruction to both the answerer and validator. Also fixed two bugs it exposed: "multifamily" (one word) slipped the filter, and the "Top N matches" label was really the retrieval-slice size (reworded to "Answer drawn from N verified records"). Filter-level regression tests added; 19 tests pass.

**Three more faults I caught by using the live product (all fixed):**
- **Unreadable paragraph answers** — one-firm replies came back as run-on prose. Two causes: the prompt only asked for "plain sentences", and the page used `textContent`, which collapses newlines. Fix: answerer returns labelled bullets per line; page renders `white-space: pre-wrap`.
